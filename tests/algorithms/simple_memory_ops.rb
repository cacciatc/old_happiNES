...
class SimpleMemoryOpsTest < Test::Unit::TestCase
	...
	def test_clear_16_bits_of_memory
		instr = [0xA9,0x00,0x85,0x00,0x85,0x01,0x02]
	...
	def test_clear_32_bits_of_memory
		instr = [0xA9,0x00,0x85,0x00,0x85,0x01,0x85,0x02,0x85,0x03,0x02]
		write_input_file(instr)
		system(".././2a03 #{@input_file} #{@temp_file}")
		f = File.new(@temp_file)
		fstring = f.readlines
	...
	def test_move_16_bits_of_memory
		...
		instr2 = [0xA5,0x00,0x85,0x02,0xA5,0x01,0x85,0x03,0x02]
	...
	def test_move_32_bits_of_memory
		...
		instr2 = [0xA5,0x00,0x85,0x04,0xA5,0x01,0x85,0x05,0xA5,0x02,0x85,0x06,0xA5,0x03,0x85,0x07,0x02]
		write_input_file(instr + instr2)
		system(".././2a03 #{@input_file} #{@temp_file}")
		f = File.new(@temp_file)
		fstring = f.readlines
	...
	def test_clear_32_bits_of_memory_iteratively
		...
		instr = [0xA2,0x03,0xA9,0x00,0x95,0x00,0xCA,0x10,0xFB,0x02]
	...
	def test_move_32_bits_of_memory_iteratively
		i = [0xA9,0x02,0x85,0x1,0x85,0x02,0x85,0x03,0x85,0x04]
		instr = [0xA2,0x03,0xB5,0x01,0x95,0x05,0xCA,0x10,0xFB,0x02]
		write_input_file(i + instr)
		system(".././2a03 #{@input_file} #{@temp_file}")
		f = File.new(@temp_file)
		fstring = f.readlines
		...
		assert_equal("02",fstring[$m_line].split(" ")[6])
		assert_equal("02",fstring[$m_line].split(" ")[7])
		assert_equal("02",fstring[$m_line].split(" ")[8])
	end
	
	def test_set_16_bit_constant
		instr = [0xA9,0x08,0x85,0x00,0xA9,0x08,0x85,0x01,0x02]
		write_input_file(instr)
		system(".././2a03 #{@input_file} #{@temp_file}")
		f = File.new(@temp_file)
		fstring = f.readlines
		f.close
		assert_equal("00",fstring[$status_line].chomp)
		assert_equal("08",fstring[$m_line].split(" ")[1])
		assert_equal("08",fstring[$m_line].split(" ")[2])
	end	
